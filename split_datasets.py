...
def random_stratify_split(df, type='true'):
...
def molecular_stratify_split(df, molecular_method, type='true'):
...
def to_split(target, padif_folder, path_to_work, method='random'):
    ...
    if method == 'scaffold':
        ...
        data_act, data_unseen_act = splitter(df=actives, type='true', molecular_method = 'scaffold')
        data_dec, data_unseen_dec = splitter(df=decoys, type='true', molecular_method = 'scaffold')
        data = pd.concat([data_act, data_dec])
        data_unseen = pd.concat([data_unseen_act, data_unseen_dec])
    elif method == 'fingerprint':
        actives = set[set['activity'] == 1]
        decoys = set[set['activity'] == 0]
        data_act, data_unseen_act = splitter(df=actives, type='true', molecular_method = 'fingerprint')
        data_dec, data_unseen_dec = splitter(df=decoys, type='true', molecular_method = 'fingerprint')
        data = pd.concat([data_act, data_dec])
        data_unseen = pd.concat([data_unseen_act, data_unseen_dec])
    else:
        data, data_unseen = splitter(df=set, type='true')
