...
print(
    f'''    
    ********************************************************************************
    Docking time process for {targetName} 
...
active_padif = padif_to_dataframe(act_f, cavity_file=f'{path_target}/cavity.atoms', actives=True)
...
decoys_padif = padif_to_dataframe(dec_f, cavity_file=f'{path_target}/cavity.atoms')
...
final_padif.to_csv(f'{path_target}/{targetName}_PADIF.csv', sep=',', index=False)

### Split datasets
### Define splitters and create folders
...
for splitter in splitters:
    ...
    to_split(target=targetName, padif_folder=path_target, path_to_work=path, method=splitter)


### Train models
padif_train(targetName, splitters, path_target)

print(
    f'''    
    ********************************************************************************
    Models training time process for {targetName} 
...
print(
    f'''    
    ********************************************************************************
    All time process for {targetName} 
